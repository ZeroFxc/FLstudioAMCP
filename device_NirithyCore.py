...
# 尝试导入其他可能可用的 FL Studio API 模块
_arrangement = _general = _patterns = _playlist = _ui = None
_device = _launchMapPages = None
...
try:
    ...
    pass
try:
    import launchMapPages as _launchMapPages
except ImportError:
    pass
...
def dispatch_command(action: str, params: dict) -> dict:
    ...
    # System commands
    elif action == "system.getAvailableActions":
        return handle_system_get_available_actions()
...
def handle_system_get_available_actions() -> dict:
    """返回本控制器支持的所有 MIDI action 命令列表（按模块分类）。"""
    return {
        "actions": {
            "transport": [
                "transport.start",
            ...
            "plugins": [
                ...
                "plugins.getPresetCount",
                "plugins.nextPreset",
                "plugins.prevPreset",
                "plugins.getColor",
            ],
            "system": [
                "system.getAvailableActions",
                "system.listFLStudioAPI",
            ],
        }
    }


def handle_system_list_fl_studio_api(params: dict) -> dict:
    """枚举 FL Studio 原生 Python API 所有模块的公开函数和属性。

    通过 dir() 反射获取 FL Studio 内置模块的所有公开接口，包括
    channels、mixer、plugins、transport、general、arrangement、patterns、
    playlist、ui、device 等模块。

    Args:
        params: 可选参数：
            - modules: 字符串列表，指定要枚举的模块（默认全部）
            - include_dunder: 是否包含 __xxx__ 形式的特殊属性（默认 False）
    """
    include_private = params.get("include_private", False)

    # 所有可用的 FL Studio API 模块
    api_modules = {
        "channels": channels,
        "mixer": mixer,
        "plugins": plugins,
        ...
        "ui": _ui,
        "device": _device,
        "launchMapPages": _launchMapPages,
    }

    requested = params.get("modules", None)
    result = {}

    for mod_name, mod in api_modules.items():
        if requested is not None and mod_name not in requested:
            continue
        if mod is None:
            result[mod_name] = {"available": False, "reason": "module not found in this FL Studio version"}
            continue
            ...
            if callable(obj):
                ...
                try:
                    if isinstance(obj, (int, float, str, bool, type(None))):
                        ...
                        attributes[name] = f"<{type(obj).__name__} len={len(obj)}>"
                    else:
                        attributes[name] = f"<{type(obj).__name__}>"
                except Exception:
                    attributes[name] = f"<{type(obj).__name__}>"
    ...
    return {
        "fl_studio_version": _general.getVersion() if _general else "unknown",
        "modules": result,
    }


def handle_flapi_call(params: dict) -> dict:
    """通用 FL Studio API 动态调用。

    通过 getattr 动态调用 FL Studio 原生 Python API 函数。
    支持 channels、mixer、plugins、transport、general、arrangement、
    patterns、playlist、ui、device、launchMapPages 等模块。

    Args:
        params: {
            "module": 模块名（如 "patterns"），
            "function": 函数名（如 "patternCount"），
            "args": 位置参数列表（可选），
            "kwargs": 关键字参数（可选），
        }
    """
    ...
    if not module_name or not func_name:
        return {"error": "缺少 module 或 function 参数"}
    ...
    if mod is None:
        return {"error": f"模块 {module_name} 不可用（可能未安装或版本不支持）"}

    # 获取属性
    obj = getattr(mod, func_name, None)
    if obj is None:
        return {"error": f"模块 {module_name} 中没有属性 {func_name}"}
    ...
    if callable(obj):
        try:
            ...
            return {"error": f"调用 {module_name}.{func_name} 失败: {e}"}
    else:
        # 非可调用属性，直接返回其值
        result = obj
